...
# Monkey-patch für group_norm: erzwingt contiguous Input
_orig_group_norm = F.group_norm
def _patched_group_norm(input, num_groups, weight, bias, eps):
    return _orig_group_norm(input.contiguous(), num_groups, weight, bias, eps)
...
class VideoFolderDataset(Dataset):
    """
    Erwartet eine Ordner-Struktur wie
    ...
    Jeder Unterordner ist genau ein Trainings-Sample.
    """
    def __init__(self, base_folder: str, num_frames: int, width: int, height: int):
        self.base = Path(base_folder)
        self.subdirs = sorted([d for d in self.base.iterdir() if d.is_dir()])
        if not self.subdirs:
            raise RuntimeError(f"Keine Unterordner in {self.base}")
    ...
    def __getitem__(self, idx):
        ...
        # Alle Frames alphabetisch sortieren (frame_0000.png, …)
        frame_files = sorted(folder.glob("*.[pj][pn]g"))   # .png / .jpg
        if len(frame_files) < self.num_frames:
            raise ValueError(
                f"{folder} enthält nur {len(frame_files)} Bilder – brauche {self.num_frames}")

        # exakt num_frames laden (hier die ersten N, alternativ random Ausschnitt)
        imgs = []
        for f in frame_files[: self.num_frames]:
            img = Image.open(f).convert("RGB").resize((self.width, self.height))
            arr = torch.from_numpy(np.array(img)).float() / 127.5 - 1.0  # → [-1,1]
            arr = arr.permute(2, 0, 1)           # [C,H,W]
            imgs.append(arr)

        pixel_values = torch.stack(imgs, dim=0)  # [T,C,H,W]
...
def export_to_gif(frames, output_gif_path, fps):
    ...
    - output_gif_path (str or Path): Path to save the output GIF.
    - fps (int): Frames per second for the GIF.
    """
    # Stelle sicher, dass output_gif_path ein Path ist und .gif als Suffix hat
    output_path = Path(output_gif_path).with_suffix(".gif")
    ...
    # Dauer pro Frame in Millisekunden berechnen
...
def main():
    ...
    OUT_DIR = Path(os.getenv("AZUREML_OUTPUTS_PATH", args.output_dir or "outputs"))
    ...
    image_encoder.to(accelerator.device, dtype=weight_dtype, memory_format=torch.contiguous_format)
    vae.to(accelerator.device, dtype=weight_dtype, memory_format=torch.contiguous_format)
    unet.to(accelerator.device, dtype=weight_dtype, memory_format=torch.contiguous_format)
    ...
    if args.gradient_checkpointing:
        # rufe _set_gradient_checkpointing mit checkpoint() als positional Arg auf
        unet._set_gradient_checkpointing(checkpoint)
    ...
    # check parameters
    if accelerator.is_main_process:
        rec_txt1 = open(OUT_DIR / 'params_freeze.txt', 'w')
        rec_txt2 = open(OUT_DIR / 'params_train.txt',  'w')
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        # UNet mit LoRA-Adapter entpacken und als reinen LoRA-Checkpoint speichern
                        unwrapped_unet = accelerator.unwrap_model(unet)
                        unwrapped_unet.save_pretrained(
                            save_path,
                            safe_serialization=True,
                        )
                        logger.info(f"Saved LoRA-Adapter in {save_path}")

                    # sample images!
                    if (global_step % args.validation_steps == 0) or (global_step == 1):
                        logger.info(f"Running validation... \n Generating {args.num_validation_images} videos.")
                        # unwrap and load pipeline
                        ...
                        # move to device in BF16 and enforce contiguous layout to avoid ChannelsLast3d issues
                        pipeline = pipeline.to(accelerator.device, dtype=weight_dtype)
                        pipeline.unet = pipeline.unet.to(
                            accelerator.device,
                            dtype=weight_dtype,
                            memory_format=torch.contiguous_format,
                        )
                        pipeline.vae = pipeline.vae.to(
                            accelerator.device,
                            dtype=weight_dtype,
                            memory_format=torch.contiguous_format,
                        )
                        ...
                        val_save_dir = OUT_DIR / "validation_images"
                        ...
                        # run inference
                        with torch.autocast(str(accelerator.device).replace(":0", ""), dtype=weight_dtype):
                            for val_img_idx in range(args.num_validation_images):
                                num_frames = args.num_frames
                                video_frames = pipeline(
                                    load_image(str(demo_path)).resize((args.width, args.height)),
                                    height=args.height,
                                    width=args.width,
                                ...
                                out_file = Path(val_save_dir) / f"step_{global_step}_val_img_{val_img_idx}.mp4"

                                for i in range(num_frames):
                                    img = video_frames[i]
                                    video_frames[i] = np.array(img)
                                export_to_gif(video_frames, str(out_file), 8)
    ...
    if accelerator.is_main_process:
        ...
        # )
        unwrapped_unet = accelerator.unwrap_model(unet)
        unwrapped_unet.save_pretrained(OUT_DIR, safe_serialization=True)
...
if __name__ == "__main__":
    main()
    # PO546_ND_VFF_USU_B000-FH1Defo_M024-hhl-RP2_URFEhh-478-25_180_45_24